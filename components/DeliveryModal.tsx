...
const DeliveryModal: React.FC<DeliveryModalProps> = ({ date, onClose, onSave, contractItems }) => {
  ...
  const handleInputChange = (index: number, field: 'kg' | 'value', value: string) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    itemInputs.forEach((input, index) => {
      ...
      if (input.kg && input.value && !isNaN(kgNumber) && kgNumber > 0 && !isNaN(valueNumber) && valueNumber > 0) {
                ...
                {contractItems.length === 0 ? (
                    ...
                    contractItems.map((contractItem, index) => (
                      <div key={contractItem.name} className="p-3 border rounded-lg bg-white shadow-sm">
                          <p className="font-semibold text-gray-800 mb-2">{contractItem.name}</p>
                          <div className="grid grid-cols-2 gap-3">
                              <div>
                                  <label htmlFor={`kg-${index}`} className="block text-xs font-medium text-gray-600 mb-1">Quilograma (Kg)</label>
                                  <input type="number" id={`kg-${index}`} value={itemInputs[index].kg} onChange={e => handleInputChange(index, 'kg', e.target.value)} min="0.01" step="0.01" placeholder="Ex: 50.5" className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-green-500 focus:border-green-500 sm:text-sm"/>
                              </div>
                              <div>
                                  <label htmlFor={`value-${index}`} className="block text-xs font-medium text-gray-600 mb-1">Valor (R$)</label>
                                  <input type="number" id={`value-${index}`} value={itemInputs[index].value} onChange={e => handleInputChange(index, 'value', e.target.value)} min="0.01" step="0.01" placeholder="Ex: 150.00" className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-green-500 focus:border-green-500 sm:text-sm"/>
                              </div>
                          </div>
                      </div>
                    ))
