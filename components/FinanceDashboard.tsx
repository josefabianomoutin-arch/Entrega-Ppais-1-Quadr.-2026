    ...
    <div className="min-h-screen bg-gray-50 text-gray-800 pb-20 font-sans">
      <header className="bg-white shadow-xl p-4 flex justify-between items-center border-b-4 border-indigo-700 sticky top-0 z-[100]">
        <div>
            ...
            <p className="text-[10px] text-gray-500 font-bold uppercase tracking-widest mt-1">Acompanhamento de Recursos e Saldos PTRES</p>
        </div>
      ...
      <main className="p-4 md:p-8 max-w-[1600px] mx-auto space-y-8">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 animate-fade-in">
            <div className="bg-white p-8 rounded-3xl shadow-xl border-b-8 border-indigo-500 flex justify-between items-center">
                <div>
                    <p className="text-[10px] font-black text-gray-400 uppercase mb-1">Recursos Recebidos (Cotas)</p>
                    <p className="text-2xl font-black text-indigo-700">{formatCurrency(totals.recurso)}</p>
                </div>
            ...
            <div className="bg-white p-8 rounded-3xl shadow-xl border-b-8 border-red-500 flex justify-between items-center">
                <div>
                    <p className="text-[10px] font-black text-gray-400 uppercase mb-1">Gasto Total Utilizado</p>
                    <p className="text-2xl font-black text-red-600">{formatCurrency(totals.utilizado)}</p>
                </div>
            ...
            <div className={`bg-white p-8 rounded-3xl shadow-xl border-b-8 flex justify-between items-center ${totals.recurso - totals.utilizado >= 0 ? 'border-green-600' : 'border-red-900'}`}>
                <div>
                    <p className="text-[10px] font-black text-gray-400 uppercase mb-1">Saldo Atual Disponível</p>
                    <p className={`text-3xl font-black ${totals.recurso - totals.utilizado >= 0 ? 'text-green-700' : 'text-red-900'}`}>{formatCurrency(totals.recurso - totals.utilizado)}</p>
                </div>
                <div className={`p-4 rounded-2xl ${totals.recurso - totals.utilizado >= 0 ? 'bg-green-50 text-green-600' : 'bg-red-100 text-red-900'}`}>
            ...
            <div className="overflow-x-auto rounded-3xl border-2 border-gray-50">
                <table className="w-full text-sm">
                    <thead className="bg-gray-900 text-white text-[10px] font-black uppercase tracking-widest">
                        <tr>
                            <th className="p-5 text-left">TIPO / PTRES</th>
                            <th className="p-5 text-left">NATUREZA</th>
                            <th className="p-5 text-left">DATA</th>
                            <th className="p-5 text-right">VALOR</th>
                            <th className="p-5 text-left">DESCRIÇÃO / PROCESSO</th>
                            <th className="p-5 text-center">STATUS</th>
                        </tr>
                    </thead>
                    <tbody className="divide-y divide-gray-100">
                        {filteredRecords.map(r => (
                            <tr key={r.id} className={`hover:bg-gray-50 transition-colors ${r.tipo === 'RECURSO' ? 'bg-indigo-50/30' : ''}`}>
                                <td className="p-5">
                                ...
                                <td className="p-5 font-bold text-gray-400">{r.natureza}</td>
                                ...
                                <td className="p-5 text-right font-mono">
                                    <p className={`font-black ${r.tipo === 'RECURSO' ? 'text-indigo-700' : 'text-red-600'}`}>
                                      {r.tipo === 'RECURSO' ? `+ ${formatCurrency(r.valorRecebido || r.valorSolicitado)}` : `- ${formatCurrency(r.valorUtilizado)}`}
                                    </p>
                                </td>
                                <td className="p-5">
                                    <p className="font-bold text-gray-800 uppercase text-xs truncate max-w-md">{r.descricao}</p>
                                    <p className="text-[10px] text-gray-400 font-mono italic">Proc: {r.numeroProcesso || '-'}</p>
                                </td>
